...
<html lang="en">
  <head>
    ...
    <title>Orange-poolvilla:insertPoolvillaOtt</title>
  ...
  <section class="probootstrap-section">
    <div class="container">
			...
			<tbody>
				<tr>
					<c:forEach var = "o" items = "${ list }" varStatus = "status">
						<form method="post" action="${pageContext.request.contextPath}/host/insertPoolvillaOttController">
						<input type = "hidden" name = "pvNo" value = "${ param.pvNo }" readonly>
							<tr>
								<td><input type = "hidden" name = "ottNo" value = "${ o.ottNo }" readonly>${ status.count }</td>
								<td><input type = "hidden" name = "ottName" value = "${ o.ottName }" readonly>${ o.ottName }</td>
				...
				<tr>
					<c:if test="${ not empty poList }">
						<c:forEach var = "po" items = "${ poList }">
								<tr>
									<td>${ po.ottName }</td>
									<td><a href="${pageContext.request.contextPath}/host/deletePoolvillaOttController?pvNo=${ param.pvNo }&ottNo=${ po.ottNo }" class = "btn btn-outline-secondary btn-sm">삭제</a></td>
								</tr>
						</c:forEach>
					</c:if>
				</tr>
		...
		<a href="${pageContext.request.contextPath}/host/ottController?pvNo=${ pvNo }" class = "btn btn-primary btn-sm">다른 ott 목록 추가하러 가기</a>
